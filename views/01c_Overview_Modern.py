...
with left_col:
    ...
    # All tickers per segment (including those without price data)
    _all_seg_tickers = {}
    for d in filtered_data:
        sk = d.get("segment")
        t = d.get("ticker")
        if sk and t:
            _all_seg_tickers.setdefault(sk, []).append(t)

    _seg_data = []
    for sk in seg_keys:
        if sk not in selected_segments:
            continue
        all_t = _all_seg_tickers.get(sk, [])
        seg_t_with_ret = [t for t in all_t if t in returns.index]
        if not seg_t_with_ret:
            continue
        # Market-cap weighted return
        seg_rets = returns[seg_t_with_ret]
        seg_weights = pd.Series({t: _ticker_mcap.get(t, 1.0) for t in seg_t_with_ret})
        ws = seg_weights.sum()
...
if not returns.empty:
    ...
    with w_col:
        ...
        if w_more or st.session_state.get("v2_show_more_w"):
            _w_expanded = st.session_state.get("v2_show_more_w", False)
            _w_label = "\u25B4 Show fewer" if _w_expanded else "\u25BE Show all 25 winners"
            if st.button(_w_label, key="v2_btn_more_w"):
                st.session_state["v2_show_more_w"] = not _w_expanded
                st.rerun()
    with l_col:
        ...
        if l_more or st.session_state.get("v2_show_more_l"):
            _l_expanded = st.session_state.get("v2_show_more_l", False)
            _l_label = "\u25B4 Show fewer" if _l_expanded else "\u25BE Show all 25 losers"
            if st.button(_l_label, key="v2_btn_more_l"):
                st.session_state["v2_show_more_l"] = not _l_expanded
                st.rerun()
